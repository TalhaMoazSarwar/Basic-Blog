    ...
    @foreach ($posts as $post)
        <div class="card mb-2">
            <div class="card-body">
                ...
                <span style="font-size: 0.85rem" class="text-secondary text-uppercase">By <span class="little-bold">{{ $post->user->name }}</span> on {{ $post->created_at->toFormattedDateString() }}</span>
            </div>
        </div>
    @endforeach
    {{ $posts->links() }}
